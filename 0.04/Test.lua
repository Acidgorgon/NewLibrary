...
-- Replace the loadstring below with the actual URL once you host your code
...
-- For testing locally, just use: local Library = require(path.to.Library)

local LocalPlayer = game:GetService("Players").LocalPlayer

local Window = Library.new(
    "NextLevel",               -- window title
    UDim2.fromOffset(688, 618),     -- size (can be omitted for default)
...
task.spawn(function()
    ...
    while task.wait(1) do
        ...
        statsTable:SetValue("Ping",       string.format("%.0f ms", p))
...
ToggleSection:ToggleBind("Toggle Aim Assist", false, Enum.KeyCode.F, "aimAssist", function(val)
    print("[Toggle] Aim Assist:", val)
end)
...
ButtonSection:CreateButtonRow(
    "Enable All", function()
        for _, flag in ipairs({"espEnabled","aimbotEnabled","autoParry","autoFarm","silentAim"}) do
            if Window.Setters[flag] then Window.Setters[flag](true) end
        end
        Window:Notify("Batch", "All features enabled.", 2)
    end,
    "Disable All", function()
        for _, flag in ipairs({"espEnabled","aimbotEnabled","autoParry","autoFarm","silentAim","noClip","speedHack","godMode","infStamina"}) do
...
ButtonSection:CreateButtonRow(
    ...
    "Print Version", function()
        Window:Notify("NextLevel", "Version 2.0.0 — Extended Edition", 3)
    end
)
...
local PlayerMods = Window:Tab("Players"):Section("Modifications")

-- Instantly auto-updates, they don't have to write any player loops!
PlayerMods:PlayerDropdown("Select Target", false, "AimbotTarget", function(player)
    print("Targeted:", player)
end)

local DebugTab = Window:Tab("Debug"):Section("Live Output")
...
-- Spawns a live terminal in the UI
local MyConsole = DebugTab:Console(150)
MyConsole:Print("System initialized.", THEMES.InfoColor)
MyConsole:Print("Bypassing anti-cheat...", THEMES.WarningColor)
MyConsole:Print("Success!", THEMES.SuccessColor)

-- ═══════════════════════════════════════════════════════════════════════════════
-- BUILT-IN SETTINGS TAB  (config save/load, macros, about)
-- ═══════════════════════════════════════════════════════════════════════════════
...
print("[NextLevel] UI loaded. Player:", LocalPlayer.Name)