...
export type LibraryModule = {
    new: (Name: string?, Size: UDim2?, KeyBind: (Enum.UserInputType | Enum.KeyCode)?, Mode: string?) -> Window,
...
function Library.new(Name: string?, Size: UDim2?, KeyBind: (Enum.UserInputType | Enum.KeyCode)?, Mode: string?): Window
...
function Library:BuildSettingsTab(iconId)
    ...
    UISetupSection:ColorPicker("Custom Accent Color", THEMES.LineColor, 1, "AccentColor", function(color, alpha)
        if not self.Flags["RGBMode"] then
            self:SetTheme({LineColor = color})
        end
    end)
